...
def test_cleanup_all_resumes():
    ...
    response = requests.post(f"{BASE_URL}/api/v1/cleanup-all-resumes/", verify=VERIFY_SSL)
    ...
    response = requests.post(f"{BASE_URL}/api/v1/cleanup-all-resumes/", data=data, verify=VERIFY_SSL)
    print(f"Status: {response.status_code}")
    print(f"Response: {response.json()}")
...
def demonstrate_production_security():
    ...
    try:
        response = requests.post(f"{production_url}/api/v1/cleanup-all-resumes/", data=data, verify=VERIFY_SSL)
