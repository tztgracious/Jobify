...
from .utils import check_file_size_with_message, get_session_by_id, parse_resume
...
def upload_resume(request):
    ...
    logger.info(
        f"Received file: {file.name}, size: {file.size} bytes, content_type: {file.content_type}"
    )

    # Check file type
    if not file.content_type == "application/pdf":
        logger.warning(
            f"Upload attempt with invalid file type: {file.content_type}, filename: {file.name}"
        )
    ...
    if not is_valid_size:
        logger.warning(
            f"Upload attempt with oversized file: {file.size} bytes, filename: {file.name}"
        )
        logger.info("=== UPLOAD RESUME REQUEST FAILED - FILE TOO BIG ===")
        return Response(
            {"id": None, "valid_file": False, "error_msg": "File too big."},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    try:
        ...
        logger.error(
            f"Failed to save resume file: {str(e)}, attempted path: {save_path}"
        )
    ...
    try:
        interview_session = InterviewSession.objects.create(
            id=session_id, resume_local_path=save_path
        )
        ...
        return Response(
            {"id": None, "valid_file": False, "error_msg": f"Database error: {str(e)}"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )

    # Log successful upload
    logger.info(
        f"Resume uploaded successfully: id={session_id}, filename={filename}, file_size={file.size} bytes"
    )
...
def get_grammar_results(request):
    ...
    if not session_id:
        ...
        return Response(
            {"finished": False, "grammar_check": None, "error": "id is required"},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    if not session:
        ...
        return Response(
            {"finished": False, "grammar_check": None, "error": "Resume not found"},
    ...
    if session.resume_status == InterviewSession.Status.FAILED:
        ...
        logger.info(
            "=== GET GRAMMAR RESULTS REQUEST - PROCESSING FAILED, RESTARTING ==="
        )
        return Response(
            {
                "finished": False,
        ...
        return Response(
            {"finished": False, "grammar_check": None, "error": ""},
...
def get_keywords(request):
    ...
    if not session_id:
        ...
        return Response(
            {"finished": False, "keywords": [], "error": "id is required"},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    if not resume:
        ...
        return Response(
            {"finished": False, "keywords": [], "error": "Resume not found"},
            status=status.HTTP_404_NOT_FOUND,
        )
    ...
    if resume.resume_status == InterviewSession.Status.FAILED:
        ...
        return Response(
            {"finished": False, "keywords": [], "error": ""}, status=status.HTTP_200_OK
        )
    elif resume.resume_status == InterviewSession.Status.COMPLETE:
        keywords = resume.keywords or []
        logger.info(
            f"Resume processing complete for id: {session_id}, keywords count: {len(keywords)}"
        )
        logger.info(f"Keywords: {keywords}")
        logger.info("=== GET KEYWORDS REQUEST COMPLETED SUCCESSFULLY ===")
        return Response(
            {"finished": True, "keywords": keywords, "error": ""},
            status=status.HTTP_200_OK,
        )
    ...
    return Response(
        {"finished": False, "keywords": [], "error": "Unknown resume status"},
        status=status.HTTP_500_INTERNAL_SERVER_ERROR,
    )
...
def target_job(request):
    ...
    logger.info(
        f"Received id: {session_id}, title: {title}, answer_type: {answer_type}"
    )

    # Validate required fields
    if not session_id or not title or not title.strip():
        logger.warning(
            f"Target job called without required fields - id: {session_id}, title: {title}"
        )
    ...
    resume.save()
    logger.info(
        f"Target job updated for id: {session_id}, new: '{title}', answer_type: '{answer_type}'"
    )
...
def cleanup_all_resumes(request):
    ...
    expected_token = os.getenv(
        "DELETE_ALL_RESUMES_TOKEN", "o2euinpiebvoian;*&Ts"
    )  # Change this periodically

    if confirmation_token != expected_token:
        logger.warning(
            f"Cleanup all resumes attempted with invalid token: {confirmation_token}"
        )
    ...
    if confirm_action != "DELETE_ALL_RESUME_DATA":
        logger.warning(
            f"Cleanup all resumes attempted without proper confirmation: {confirm_action}"
        )
        logger.info(
            "=== CLEANUP ALL RESUMES REQUEST BLOCKED - MISSING CONFIRMATION ==="
        )
        return Response(
            {
                "success": False,
    ...
    try:
        ...
        logger.info(
            f"Database cleanup completed. Records removed: {db_records_removed}"
        )
