...
class UploadResumeTests(APITestCase):
    ...
    def test_upload_resume_success(self):
        ...
        pdf_file = SimpleUploadedFile(
            "test_resume.pdf", self.valid_pdf_content, content_type="application/pdf"
        )
        ...
        expected_path = os.path.join(
            settings.MEDIA_ROOT, "resumes", f"{session_id}.pdf"
        )
...
class TargetJobTests(APITestCase):
    ...
    def test_target_job_success(self):
        ...
        response = self.client.post(
            self.url, {"id": session_id, "title": "Software Engineer"}
        )

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["id"], session_id)
        self.assertEqual(
            response.data["message"], "Target job and answer type saved successfully"
        )
    ...
    def test_target_job_with_video_answer_type(self):
        ...
        response = self.client.post(
            self.url,
            {"id": session_id, "title": "Software Engineer", "answer_type": "video"},
        )
    ...
    def test_target_job_missing_id(self):
        ...
        response = self.client.post(self.url, {"title": "Software Engineer"})
    ...
    def test_target_job_invalid_answer_type(self):
        ...
        response = self.client.post(
            self.url,
            {"id": session_id, "title": "Software Engineer", "answer_type": "invalid"},
        )

        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.data["error"], "answer_type must be either 'text' or 'video'"
        )

    def test_target_job_invalid_id(self):
        """Test target job with non-existent id"""
        response = self.client.post(
            self.url, {"id": "invalid-uuid-12345", "title": "Software Engineer"}
        )
...
class RemoveResumeTests(APITestCase):
    ...
    def test_remove_resume_missing_file(self):
        ...
        self.assertEqual(response.data["message"], "Resume removed successfully")
        self.assertFalse(
            response.data["file_removed"]
        )  # File wasn't removed because it didn't exist
